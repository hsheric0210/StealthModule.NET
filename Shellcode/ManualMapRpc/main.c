...
typedef HMODULE(WINAPI *FuncLoadLibraryA) (_In_z_	LPTSTR lpFileName);


typedef int (WINAPI *FuncMessageBoxA) (_In_opt_ HWND hWnd, _In_opt_ LPCSTR lpText, _In_opt_ LPCSTR lpCaption, _In_ UINT uType);

typedef NTSTATUS(WINAPI *FuncRtlEncryptMemory) (_Inout_ PVOID Memory, _In_    ULONG MemoryLength, _In_    ULONG OptionFlags);

typedef NTSTATUS(WINAPI *FuncRtlDecryptMemory) (_Inout_ PVOID Memory, _In_    ULONG MemoryLength, _In_    ULONG OptionFlags);

typedef VOID(WINAPI *FuncSleep) (_In_ DWORD dwMilliseconds);

typedef BOOL(WINAPI *FuncVirtualFree) (_In_ LPVOID lpAddress, _In_ SIZE_T dwSize, _In_ DWORD  dwFreeType);
